...
#include "./XAITimer.hpp"

XAICScopedTimer::XAICScopedTimer(const std::string& task, XAICTimer* timer): t(timer) {
	t->timerDepth += 1;
...
XAICScopedTimer::~XAICScopedTimer() {
	td.t2 = SDL_GetTicks();
	td.t3 = td.t2 - td.t1;
...
unsigned int XAICTimer::GetTaskTime(const std::string& t) {
...
void XAICTimer::WriteLog() {
	/*
	// too much output
	for (std::vector<XAICScopedTimer::TimingDatum>::const_iterator it = vtimings.begin(); it != vtimings.end(); it++) {
		const XAICScopedTimer::TimingDatum& td = *it;
